    ...
    public @NotNull InteractionResult use(@NotNull Level level, Player player, @NotNull InteractionHand interactionHand) {
        ...
        if (!player.getActiveEffects().isEmpty()) {
            ...
            if (!level.isClientSide() && player instanceof ServerPlayer serverPlayer) {
                ...
                serverPlayer.playNotifySound(CSoundEvents.CLEAR_EFFECTS_USE.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
                serverPlayer.gameEvent(GameEvent.ITEM_INTERACT_FINISH);
                ParticleHelper.emitParticlesAroundEntity(serverPlayer, ParticleTypes.EFFECT, 1.0D, 10);
